...
const storyPhases = [
  { progress: [0.05, 0.12], label: 'HARDWARE', visual: 'laptop-closed' },
  { progress: [0.12, 0.22], label: 'ACTIVATING', visual: 'laptop-open' },
  { progress: [0.22, 0.32], label: 'CIRCUITRY', visual: 'motherboard' },
  { progress: [0.32, 0.42], label: 'PROCESSING', visual: 'processor' },
  { progress: [0.42, 0.52], label: 'DATA', visual: 'sql' },
  { progress: [0.52, 0.62], label: 'CODE', visual: 'code' },
  { progress: [0.62, 0.72], label: 'CONTAINERS', visual: 'docker' },
  { progress: [0.72, 0.82], label: 'VIRTUALIZATION', visual: 'proxmox' },
  { progress: [0.82, 0.95], label: 'ECOSYSTEM', visual: 'ecosystem' },
];

const transitionTexts = [
  { text: 'Systems designed for reliability.', start: 0.15, end: 0.25 },
  { text: 'Automation designed for efficiency.', start: 0.35, end: 0.45 },
  { text: 'Infrastructure designed for growth.', start: 0.55, end: 0.65 },
  { text: 'Technology designed for business.', start: 0.7, end: 0.8 },
  { text: 'Everything connected.', start: 0.85, end: 0.95 },
];
  ...
  return (
    <section ref={containerRef} className="relative h-[600vh]">
      <div className="sticky top-0 h-screen flex items-center justify-center overflow-hidden">
        ...
        {/* Phase indicator */}
        <PhaseIndicator scrollYProgress={scrollYProgress} phases={storyPhases} />

        {/* Transition texts */}
        {transitionTexts.map((item, i) => (
          <TransitionText
            key={i}
            text={item.text}
            start={item.start}
      ...
      {phases.map((phase, i) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-[9px] font-mono text-brand-muted tracking-wider">{phase.label}</span>
          </motion.div>
        );
      })}
    </div>
