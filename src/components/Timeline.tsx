  ...
  return (
    <section className="relative py-40 px-6">
      ...
      <div className="max-w-4xl mx-auto" ref={ref}>
        ...
        >
          <p className="text-brand-accent font-mono text-xs mb-6 tracking-[0.3em] uppercase">{t('timeline.label')}</p>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold">
            {t('timeline.title')} <span className="text-gradient">{t('timeline.titleAccent')}</span>
          </h2>
        </motion.div>
                ...
                >
                  ...
                  <div className={`md:w-1/2 ${isLeft ? 'md:text-right md:pr-12' : 'md:pl-12'}`}>
                    <div className="glass rounded-xl px-6 py-5 inline-block hover:glow-box transition-all duration-500">
                      <h3 className="text-base font-semibold text-white mb-1">{t(`timeline.milestones.${key}.label`)}</h3>
                      <p className="text-xs text-brand-muted">{t(`timeline.milestones.${key}.desc`)}</p>
                    </div>
                  </div>
